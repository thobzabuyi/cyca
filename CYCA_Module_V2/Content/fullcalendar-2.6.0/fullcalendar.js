...
// set `preserUserime` to `true` to keep times, but only normalize zone ambiguity.
// returns the original moments if no modifications are necessary.
function commonlyAmbiguate(inputs, preserUserime) {
	...
	for (i = 0; i < len; i++) {
		mom = moms[i];
		if (!preserUserime && anyAmbigTime && !mom._ambigTime) {
