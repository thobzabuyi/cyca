...
namespace Common_Objects.Models
{
    public class PersonModel
    {
        ...
        public Person GetPersonByGuid(Guid g)
        {
           return db.Persons.Where(p => p.DSD_Uid == g).FirstOrDefault();
        }
        public IEnumerable<Person> GetExistingPeople()
        {
           return (from p in db.Persons
             join cc in db.Clients on p.Person_Id equals cc.Person_Id
             where p.Is_Active || p.Is_Active.Equals(!false)
             where !p.Is_Deleted || p.Is_Deleted.Equals(false)
        ...
        public List<Person> GetListOfPersonsPCM(bool showInActive, bool showDeleted)
        {
            ...
            try
            {
                ...
                var persons = (from p in dbContext.Persons
                               ...
                               //join f in db.int_Client_Module_Registration on t.Client_Id equals f.Client_Id
                               where p.Is_Active || p.Is_Active.Equals(!showInActive)
                               && !p.Is_Deleted || p.Is_Deleted.Equals(showDeleted)
                               ////&& q.Problem_Sub_Category_Id == 22
                               //&& t.int_Client_Module_Registration.FirstOrDefault().Client_Module_Ref_No != null
                               select p).ToList().Distinct();
